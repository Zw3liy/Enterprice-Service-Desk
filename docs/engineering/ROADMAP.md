...
Current as of milestone **SEC-01**. Update item status in place as work lands; don't leave completed
...
3. **`OPEN` — possible RBAC gap, needs a decision, not a silent fix.** `get_ticket_queryset`'s Technician
